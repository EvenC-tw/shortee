...
function isValidWebUrl(urlObject: URL): boolean {
  ...
  if (hostname === "localhost" || !hostname.includes(".")) {
    console.debug(
      `isValidWebUrl: 主機名稱無效 (缺少 TLD 或為 localhost) - ${hostname}`
    );
    return false;
  }
  return true;
...
function Home({ isDarkMode, toggleTheme, drawerVisible, setDrawerVisible }) {
  ...
  async function postShortee(): Promise<void> {
    ...
    try {
      ...
      valid = isValidWebUrl(parsedUrl);
      finalUrl = parsedUrl.href;
    } catch {
      // 嘗試自動補 https
      try {
        parsedUrl = new URL(`https://${urlInput.trim()}`);
        valid = isValidWebUrl(parsedUrl);
        finalUrl = parsedUrl.href;
      } catch {
        try {
          parsedUrl = new URL(`http://${urlInput.trim()}`);
          valid = isValidWebUrl(parsedUrl);
          finalUrl = parsedUrl.href;
        } catch {
          valid = false;
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "處理請求時發生未知網路錯誤。" }));
        messageApi.error(
          errorData.message || `伺服器錯誤，狀態碼: ${response.status}`
        );
        setShorteeing(false);
        return;
      }
      ...
      messageApi.error(
        (error as Error).message || "建立短網址時發生預期外的錯誤，請稍後再試。"
      );
    } finally {
      setShorteeing(false);
    }
                            ...
                            <div style={{ width: "100%" }}>
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    copyShortUrl(
                                      `${location.origin}/${item.shorteeCode}`
                                    )
                                  }
                                  style={{ marginLeft: 8 }}
                                />
                              ...
                              >
                                <Button
                                  ...
                                  onClick={() =>
                                    setShowOriginIndex(
                                      showOriginIndex === index ? null : index
                                    )
                                  }
                                >
